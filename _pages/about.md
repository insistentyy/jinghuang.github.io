...
- **语言能力：** 已通过 CET4 和 CET6，有作为独立第一作者撰写并投稿全英文学术论文（2篇，均已录用）的经历  
...
- **我们的方法：**  
  1. **引入预训练嵌入蒸馏：** 我们提出了基于预训练嵌入蒸馏的字典时序图网络 (DicTED)，聚合多个预训练教师模型生成的嵌入作为先验特征，有效解决了冷启动和灾难性遗忘问题。
  2. **优化策略：** 在输入端，结合先验特征和原始特征增强模型的初始化；在优化端，通过嵌入损失和预测分数损失保留原始信息，提升模型性能。
- **项目成果：**  
  * 在多个真实数据集上，DicTED展示了优于现有SOTA方法的性能。
  * 作为独立第一作者投稿的论文已被ICIC 2024 (CCF-C) 录用并发表。
...
- **角色：** 团队(主力成员)  
- **研究背景：**  
  母亲的心理健康状况对婴儿的认知和情感发展有显著影响。分析和建模母亲的身体和心理指标与婴儿睡眠质量之间的关系，有助于提升母亲的心理健康水平和婴儿的生理和心理健康。
- **我们的论文：**  
...
- **角色：** 团队(主力成员)  
- **研究背景：**  
  传统海洋测绘采用单波束测深系统，但在现代海洋测绘中，高效、高精度的多波束测深系统被更广泛应用。为确保数据精确，设计合适的测线布局非常重要。
- **我们的论文：**  
...
- **角色：** 团队(主力队员)  
- **研究背景：**  
  无人机在民用和军用中广泛应用，尤其在军用中，无人机常以编队形式执行任务，为避免被雷达探测，应减少电磁信号传播。如何通过无源探测定位调整无人机位置显得尤为重要。
- **我们的论文：**  
